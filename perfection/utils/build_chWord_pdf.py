...
class PDF_TEMPLATES:
    class REMEMBER:
        ...
        LINE = '<para spaceAfter=0 spaceBefore=0 leading=12>' \
               '<font face="霞鹜文楷" size=12>{index}. {sentence} <b>{key}</b>：</font>' \
    ...
    class REVIEW:
        ...
        LINE = '<para>' \
               '<font face="霞鹜文楷" size=12>{index}. {sentence} {key} _______________</font>' \
               '<br/></para>'
        LINE_STYLE = REVIEW_LINE_STYLE

        NO_WORDS = '<para><font face="霞鹜文楷">今天没有文言字词</font></para>'
            ...
            body_remember.append(
                Paragraph(
                    PDF_TEMPLATES.REMEMBER.LINE.format(
                        index=index + 1,
                        key=word.chWord.key,
                        sentence=word.chWord.sentence,
                        value=word.chWord.value
